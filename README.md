...
*   **Mulligan Practice:** Simulate "Going First" (3 cards) or "Going Second" (4 cards + The Coin). Test your keep/replace decisions.
*   **Sandbox Gameplay:** Draw cards, play cards (mana tracking), and end turns to see how your curve plays out.
*   **Advanced Deck Manipulation:**
    *   **Discover:** Find cards in your remaining deck (with filters for Minions or Spells). Option to take the card or generate a copy.
    *   **Dredge:** Inspect the bottom 3 cards and pick one to place on top.
    *   **Swap:** Swap your entire hand with the bottom of your deck. (The effect of Sir Finley, Sea Guide)
    *   **Specific Draw:** Tutor the next Minion or Spell from the top of your deck.
...
### 1. Getting Started
1.  Copy a **Deck String** from Hearthstone or a deck-building website (starts with `AAEC...`).
...
*   **Swap:** Places your current hand at the bottom of the deck and draws the same number of cards from the bottom.
...
*   **沙盒操作：** 自由抽牌、打出卡牌（自動計算費用）、結束回合，觀察水晶曲線是否順暢。
*   **進階牌庫操作：**
    *   **發現 (Discover)：** 從剩餘牌庫中隨機展示3張牌（可限定手下或法術）。可選擇直接拿取（從牌庫移除）或獲得複製。
    *   **探底 (Dredge)：** 檢視牌庫底部的3張牌，並選擇一張置頂。
    *   **交換 (Swap)：** 將當前手牌與牌庫底部的卡牌進行交換。(『海底嚮導』芬利的效果)
    *   **指定抽牌：** 直接抽出牌庫頂的第一張「手下」或「法術」。
*   **拖曳手勢：** 支援直覺的拖放操作來丟棄卡牌、檢索牌庫或將卡牌置底。
...
### 1. 開始使用
1.  從爐石戰記遊戲內或任何牌組網站複製 **套牌代碼**（以 `AAEC...` 開頭）。
2.  貼上到頂部的輸入框中。
3.  點擊 **載入套牌**。
...
*   **發現 (Discover)：** 從牌庫中三選一。
    *   *點擊卡圖：* 將該牌從牌庫移入手牌。
    *   *點擊「獲得複製」：* 獲得一張複製品，原牌保留在牌庫中。
*   **交換 (Swap)：** 將目前所有手牌放到牌庫底，並從牌庫底換上等量的牌。
*   **結束回合：** 增加回合數並重置已消耗費用。

### 4. 拖曳操作 (Drag & Drop)
*   **丟棄 (Discard)：** 將卡牌從 **手牌區** 拖曳到空白背景處。
*   **摧毀 (Destroy)：** 將卡牌從 **右側牌庫列表** 拖曳到空白背景處（直接移除該牌）。
*   **檢索 (Tutor)：** 將卡牌從 **右側牌庫列表** 拖曳到 **手牌區**（定向檢索該張牌）。
